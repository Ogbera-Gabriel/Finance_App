  ...
  .get(
    ...
    async (c) => {
      ...
      const data = await db
        ...
        .from(transactions)
        .leftJoin(categories, eq(transactions.categoryId, categories.id)) // left join because categories is not required
        .innerJoin(accounts, eq(transactions.accountId, accounts.id)) //inner join because accounts is required
        .where(
          and(
